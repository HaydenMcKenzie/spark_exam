...
import Schemas.{RawAccountSchema, RawCustomerSchema}
...
class JoinDataTest extends AnyFunSuite with BeforeAndAfter {
  ...
  test("Test joinData function on fixed data") {
    // Sample input data
    val firstData = RawCustomerSchema("1", "Alice", "Smith")
    val firstTestingData: Dataset[RawCustomerSchema] = Seq(firstData).toDS()

    val secondData = RawAccountSchema("1", "ACC001", 100.0)
    val secondTestingData: Dataset[RawAccountSchema] = Seq(secondData).toDS()

    // Call the function
    val result = JoinData.joinData(firstTestingData, secondTestingData, "customerId", "left")
  ...
  test("Test joinData function on multiple fixed data") {
    ...
    // Call the function
    val result = firstData.join(secondData, "customerId")
  ...
  test("Test joinData function must be 545 lines of data") {
    // Input data
    val firstData = readFileData[Schemas.RawCustomerSchema]("customer_data")
    val secondData = readFileData[Schemas.RawAccountSchema]("account_data")

    // Call the function
    val joinedData: Dataset[_] = JoinData.joinData(firstData, secondData, "customerId", "Left")
  ...
  test("Test joinData function Schema") {
    ...
    // Call the function
    val joinedData= firstData.join(secondData, "customerId")
